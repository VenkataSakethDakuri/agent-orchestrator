# Azure + Daytona — Cloud Agents v1 Plan

**Status:** Plan (supersedes `hybrid-cloud-agents.md` for v1)
**Scope:** Single operator, pure-cloud execution. Prove the cloud-agent loop with the least moving parts.

> This plan reflects decisions made in planning and **supersedes the rev3 design doc** where they conflict. The design doc specifies Postgres + an `ssh`-to-VM provider + mandatory auth; v1 below uses **SQLite + Daytona + a shared token**, all deferred upgrades.
...
15. Route all spawns to the Daytona adapters (`runtimeselect`); inject `GITHUB_TOKEN` + model key at sandbox create (env/secrets, never in the image).
